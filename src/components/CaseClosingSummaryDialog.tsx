...
import {
  ...
  fetchCaseClosingSummary,
  generateCaseClosingEvaluation,
} from "../services/caseClosingApi";
import type {
...
export function CaseClosingSummaryDialog({ caseId, onClose, open }: Props) {
  ...
  async function requestEvaluation(): Promise<void> {
    ...
    try {
      ...
      setSummary((current) =>
        current ? { ...current, evaluation: nextEvaluation } : current,
      );
    } catch (err) {
  ...
  const availableDocuments =
    summary?.documents.filter((item) => item.available) || [];
      ...
      {summary ? (
        <section className="closing-summary-section">
          ...
          <div className="closing-stat-grid">
            <span>
              <strong>{summary.playerTurnCount}</strong>玩家提交
            </span>
            <span>
              <strong>{summary.documentCount}</strong>可下载文书
            </span>
            <span>
              <strong>{availableDocuments.length}</strong>已生成 PDF
            </span>
          </div>
          <div className="closing-document-list">
            ...
            {!availableDocuments.length ? (
              <div className="empty-case">暂无可下载文书。</div>
            ) : null}
          </div>
        </section>
      ) : null}
    ...
    <div className="closing-workbench-main">
      {summary ? (
        <section className="closing-summary-section">
          ...
          <div className="closing-report-note">
            ...
            <span>
              条玩家提交已纳入评分。完整提交与对话请导出复盘 Markdown 查看。
            </span>
          </div>
        </section>
      ) : null}
      ...
      <section className="closing-evaluation-section">
        ...
        {evaluation ? (
          ...
          <div className="closing-evaluation-loading">
            AI 正在基于本轮提交生成评价...
          </div>
        ) : evaluationError ? (
