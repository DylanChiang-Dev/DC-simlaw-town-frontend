...
assert.match(
  appSource,
  /const \[autoOpenedPlayerRequestId, setAutoOpenedPlayerRequestId\] = useState\(''\);[\s\S]*const \[acknowledgedDialogueEntryId, setAcknowledgedDialogueEntryId\] = useState\(''\);[\s\S]*const playerDialogMayAutoOpen = !latestDialogueEntry \|\| latestDialogueEntry\.id === acknowledgedDialogueEntryId;[\s\S]*useEffect\(\(\) => \{[\s\S]*if \(!activePlayerRequest\?\.requestId\) return;[\s\S]*if \(autoOpenedPlayerRequestId === activePlayerRequest\.requestId\) return;[\s\S]*if \(!playerDialogMayAutoOpen\) return;[\s\S]*setPlayerDialogOpen\(true\);[\s\S]*setAutoOpenedPlayerRequestId\(activePlayerRequest\.requestId\);/,
  'A new active player-lawyer task should automatically open once, but only after the latest role dialogue is acknowledged.',
);
